...
`eatMap` is a helper package that transforms the input data frame from the [BT-ShinyApp](https://github.com/iqb-research/BT-ShinyApp) into an interactive widget using JavaScript. It is a small R wrapper around a JavaScript/htmlwidgets map of Germany for displaying IQB Bildungstrend
results inside a Shiny application.


## Installation

You can install the development version of `eatMap` from [GitHub](https://github.com/iqb-research/eatMap) with:
...
- an `htmlwidget` that can be embedded in Shiny via `renderEatMap()` and `eatMapOutput()`
...
knitr::include_graphics("./vignettes/images/eatMap_example.png")
```
...
3. Make changes in the JavaScript source files. JavaScript changes should be made in `srcjs/modules`.
...
The command `packer::bundle()` writes the bundled JavaScript files to `inst/htmlwidgets`. These bundled files are the files used by the R htmlwidget.
